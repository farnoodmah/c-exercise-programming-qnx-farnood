...
MsgQueueSender::MsgQueueSender(const std::string & filename): _file_name(filename){
        ...
        remove(msg_queue_name.c_str());
        ret = mq_unlink(msg_queue_name.c_str());
      ...
      //opening the file
        ...
        while (true){
            ...
            if(ret<0){
                if (errno == ETIMEDOUT){
                std::cout<<"          "<<strerror(errno)<<std::endl;
                throw IPCException("          PCSender ERROR: Cannot Connect to the Receiver");
                break;
            }
            std::cout<<strerror(errno)<<std::endl;
...
void MsgQueueReceiver::msgqTransfer(){
        ...
        while (true){
            ...
            if(_receive_size<=0){
                ...
                if (errno == ETIMEDOUT){
                std::cout<<"          "<<strerror(errno)<<std::endl;
                throw IPCException("          PCSender ERROR: Cannot Connect to the Sender");
                break;
            }
